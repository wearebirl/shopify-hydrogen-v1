   ...
   let  vocuerColours = [
    ...
    function nextStep() {
        if (selectdVoucher !== null) {
            ...
            return redirect('/birl/your-item-details', {
                headers: {
    ...
    <div>
      ...
      <BirlHeading headingText={"Voucher Credit"}></BirlHeading>
      <BirlTradeinProgress step={2}></BirlTradeinProgress>
            ...
            <div className={"flex flex-row mt-[20px] mr-[20px] space-x-3 my-auto"}>
                {vouchers.map((voucher, index) => (
                    ...
                    <div className={` ${selectdVoucher === index ? "block w-80 h-48" : "hidden md:block w-60 h-36"} my-auto  relative mb-[20px]`}  onClick={()=>setUserSelectedVoucer(index)}>
                        <div className={`${selectdVoucher === index ? "block w-80 h-48" : "hidden md:block w-60 h-36"}  left-0 top-0 absolute bg-lime-500 bg-gradient-to-tr from-${vocuerColours[index].fromColour} to-${vocuerColours[index].toColour} rounded-2xl shadow border border-white`} />
